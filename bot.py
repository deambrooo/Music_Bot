...
async def play_audio(voice_client, url, volume=1.0, replay=False):
    ...
    def after_playing(error):
        ...
        if hasattr(voice_client, "player_controls"):
            if voice_client.player_controls.looping:
                asyncio.run_coroutine_threadsafe(
                    play_audio(voice_client, url, voice_client.player_controls.volume, replay=True),
                    client.loop
                )
                return
